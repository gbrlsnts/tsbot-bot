...
export enum BotError {
    ...
    NoRootChannel = 'NoRootChannel',
    SubchannelNotOfRoot = 'SubchannelNotOfRoot',
}
...
export const invalidConfigurationError = (): Failure<
    BotError.InvalidConfiguration
> => ({
...
export const invalidChannelZoneError = (): Failure<
    BotError.InvalidChannelZone
> => ({
    type: BotError.InvalidChannelZone,
    reason: 'The channel does not exist in the zone',
});

export const invalidServerGroupError = (): Failure<
    BotError.InvalidServerGroup
> => ({
...
export const channelNameExistsError = (
    name: string
): Failure<BotError.ChannelNameExists> => ({
    type: BotError.ChannelNameExists,
    reason: `A channel with this name (${name}) already exists at the same depth`,
});

export const onlyOneSubchannelError = (): Failure<
    BotError.CannotDeleteOnlyOneSubchannel
> => ({
    type: BotError.CannotDeleteOnlyOneSubchannel,
    reason: 'Can not delete channel since there is only one subchannel',
});

export const deletingNoRootChannelError = (): Failure<
    BotError.NoRootChannel
> => ({
    type: BotError.NoRootChannel,
    reason: 'The root channel must be provided when deleting sub channel',
});

export const subchannelDoesntBelongToRootError = (): Failure<
    BotError.SubchannelNotOfRoot
> => ({
