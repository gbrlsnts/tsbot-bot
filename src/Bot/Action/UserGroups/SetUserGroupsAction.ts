...
export default class SetUserGroupsAction implements ActionInterface<boolean> {
    ...
    constructor(
        ...
        private readonly data: SetUserGroupsData
    ) {
    ...
    async execute(): Promise<Either<Failure<BotError>, boolean>> {
        ...
        const client = await this.bot.getClientByDatabaseId(
            this.data.clientDatabaseId
        );

        if (!client) return left(invalidClientError());

        const { toAdd, toRemove } = this.getGroupsToChange(
            client.servergroups || [],
            allowedGroups
        );
