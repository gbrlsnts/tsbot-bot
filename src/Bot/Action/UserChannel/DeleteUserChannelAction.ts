...
import {
    ...
    notConnectedError,
    invalidChannelError,
    onlyOneSubchannelError,
    deletingNoRootChannelError,
    subchannelDoesntBelongToRootError,
} from '../../Error';
import { Bot } from '../../Bot';
...
export class DeleteUserChannelAction
    extends ChannelAction
    implements ActionInterface<boolean> {
    constructor(
        logger: Logger,
        bot: Bot,
        private readonly data: DeleteChannelData
    ) {
        super(logger, bot);
    }
    ...
    async deleteSeparator(): Promise<void> {
        if (!this.data.zone.separators) return;
        ...
        if (index < 0 || channelList[index].pid !== 0) return;
        ...
        const posInZone = result.value.channels.findIndex(
            c => c.cid === this.data.channelId
        );

        // first. dont remove spacers
        if (posInZone === 0) return;

        // delete spacer before
        await this.bot.deleteChannel(
            result.value.channels[posInZone - 1].cid,
            true
        );
    }
    ...
    async validateChannel(
        channelId: number
    ): Promise<Failure<BotError> | undefined> {
        ...
        if (!channel) return invalidChannelError();

        if (channel.pid === 0) return;

        if (!this.data.rootChannelId) return deletingNoRootChannelError();

        const root = ChannelUtils.getRootChannelBySubChannel(
            channel,
            channelList
        );

        if (root.cid !== this.data.rootChannelId)
            return subchannelDoesntBelongToRootError();

        if (
            channel.pid === root.cid &&
            ChannelUtils.getAllSubchannels(root, channelList).length === 1
        )
